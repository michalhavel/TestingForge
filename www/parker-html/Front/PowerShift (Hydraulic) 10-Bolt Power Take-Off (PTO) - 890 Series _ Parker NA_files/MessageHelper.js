 ...
 */
     ...
     */
		displayErrorMessage:function(msg, topOffset,showType){	
			...
			$("#MessageArea .modal-footer").html('<button id="doNotRemove-button" tabindex = "1" type="button" class="btn btn-primary btn-black btn-xs-block" onclick="JavaScript:MessageHelper.hideMessageArea();toggleMiniShopCartDropDownBefore();return false;" data-dismiss="modal">OK</button>');
     ...
     */
		displayStatusMessage:function(msg,topOffset,msgContent){
			if (topOffset==null || topOffset==undefined) {
				topOffset = 0;
			}
			this.setMessageAreaStyle('1');
			// Customization done to show Go to cart and continue shopping button after adding item to the cart.
			if(msgContent != undefined && msgContent != null && msgContent!=""){
				dojo.byId('ErrorMessageText').innerHTML = msg;
				$("#MessageArea .modal-footer").html(msgContent);
			}else{
				dojo.byId('ErrorMessageText').innerHTML = msg;
			}
			// end
		...
		formErrorHandleClient:function(id,errorMessage){
			...
			if(element){
				if (this.identifier != (id + "_tooltip")) {
					...
					// Below code is used to remove message when user try to change element value. 
					dojo.connect(element, "onkeydown",  tooltip, "close");
					dojo.connect(element, "onkeydown",  tooltip, "destroy");
					dojo.connect(element, "onkeydown",  this, "clearCurrentIdentifier");
...
$(window).load(function(){
	var h=$(".returningCustomer").innerHeight();
	$(".newCustomer").css("min-height",h); 
});
