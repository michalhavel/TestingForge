...
function setLoginFlag(userType,environment){
	...
	dojo.cookie(environment+"_login_flag", userCookieValue, {path:"/",domain:domainValue});
}

function setUserInfo(userType,environment,userInfo){
	var currentHost = window.location.hostname;
	var domainValue = "";
	var isHostContainsCom = currentHost.indexOf("com");
	var userFirstName = userInfo.firstName;
	var userLastName = userInfo.lastName;
	var userFirstNameCookie = getCookie(environment+"_userFirstName");
	var userLastNameCookie = getCookie(environment+"_userLastName");
	var userWCTokenCookie = getCookie(environment+"_WCToken");
	var userWCTrustedTokenCookie = getCookie(environment+"_WCTrustedToken");
	var customerIdCookie = getCookie(environment+"_customerId");
	var customerNameCookie = getCookie(environment+"_customerName");
	var customerTypeCookie = getCookie(environment+"_customerType");
	var authorizationCodesCookie = getCookie(environment+"_authorizationCodes");
	if(isHostContainsCom != -1){
		domainValue = ".parker.com";
	}else{
		domainValue = ".parker.corp";
	...
	if (userType != "G")
	{
		dojo.cookie(environment+"_userFirstName", userFirstName, {path:"/",domain:domainValue});
		dojo.cookie(environment+"_userLastName", userLastName, {path:"/",domain:domainValue});
	} else {
		if(userFirstNameCookie != null){
			dojo.cookie(environment+"_userFirstName", null, {expires:-1,path:'/',domain:domainValue});
		}
		if(userLastNameCookie != null){
			dojo.cookie(environment+"_userLastName", null, {expires:-1,path:'/',domain:domainValue});
		}	
		if(userWCTokenCookie != null){
			dojo.cookie(environment+"_WCToken", null, {expires:-1,path:'/',domain:domainValue});
		}
		if(userWCTrustedTokenCookie != null){
			dojo.cookie(environment+"_WCTrustedToken", null, {expires:-1,path:'/',domain:domainValue});
		}
		if(customerIdCookie != null){
			dojo.cookie(environment+"_customerId", null, {expires:-1,path:'/',domain:domainValue});
		}
		if(customerNameCookie != null){
			dojo.cookie(environment+"_customerName", null, {expires:-1,path:'/',domain:domainValue});
		}
		if(customerTypeCookie != null){
			dojo.cookie(environment+"_customerType", null, {expires:-1,path:'/',domain:domainValue});
		}
		if(authorizationCodesCookie != null){
			dojo.cookie(environment+"_authorizationCodes", null, {expires:-1,path:'/',domain:domainValue});
		}
	}
}