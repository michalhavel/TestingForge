...
CheckoutHelperJS={
	...
	tempOrderTotal: "0",
	/**
	* The current orderitem id to delete from cart.
	*/
	removeOrderItemId: "",
	/**
	* The current orderitem wishlist to delete from cart.
	*/
	removeWishlist: "",

	
	/**
	 * This variable stores the value that indicates if the current page is the Quote Cart page. It is used to determine what page to redirect to after a service call has been successfully performed.
	 *
	 * @private
	 */
	quoteCartPage:false,
	...
	/**
	 * This Array store values to update the quantity of an orderItem. Its default value is empty.
	 */
	tempUpdateQuanityValues:[],
	...
	*/	
	...
	setCurrentQuoteId:function(quoteId){
	...
	deleteFromCart:function(orderItemId,forWishlist){ 
		...
		if(this.shoppingCartPage){
			params.calculationUsage = "-1,-5,-6,-7";
		}else{
			params.calculationUsage = "-1,-3,-4,-5,-6,-7";
		}
		params.check="*n";
	...
	var form = document.forms["OrderReview"];
		...
		if(!validateFormOrderReview(form)){
			return;
		} else{
		...
		//cursor_wait();
		$("#loader").fadeIn("slow");
		if($("#singleOrderSummary")!=null && $("#singleOrderSummary")!='undefined'){
				$("#singleOrderSummary").attr("disabled", true);
		}
		if($("#singleOrderSummary1")!=null && $("#singleOrderSummary1")!='undefined'){
			$("#singleOrderSummary1").attr("disabled", true);
		}
		//document.location.href = "OrderShippingBillingConfirmationView?storeId=" + this.storeId + "&catalogId=" + this.catalogId + "&langId=" + this.langId + "&orderId=" + orderId + "&shipmentTypeId=1";
	...
	showHideDivs:function(showArea,hideArea){
		// alert('showHideDivs---showArea,hideArea---'+showArea+hideArea);
	...
	editAddress:function(addressSelectBoxName,orderItemId,profileshipping,profilebilling){
		...
		//var orderID = document.getElementById(orderIdforTaxExempt);
		...
		// alert('editAddress----'+addressBox.value);
		...
		// alert('editAddress >> before wc.render.updateContext---addressBox.value---'+addressBox.value);
	...
	updateCart:function(quantityBox, orderItemId,keyPressCountValue){
		...
		if(!isNonNegativeInteger(quantity)){
			...
			$("#elmChkBoxError").html(MessageHelper.messages["CART_QTY_ERROR"]);
		} else {
			...
			// Setting the cursor focus at the end character
			setTimeout(function() {					
				var elemLen = quantityBox.value.length;
		        var input = document.getElementById (quantityBox.id);
				document.getElementById(quantityBox.id).value = input.getAttribute('value');
		        if ('selectionStart' in input) {		          
		            input.selectionStart = elemLen;
		            input.selectionEnd = elemLen;           
		        } 
		        document.getElementById(quantityBox.id).focus();
				toggleMiniShopCartDropDownBefore();
			},2000);
		}
	},
	sendCartMail:function(){
          var sender=document.forms["cartEmailForm"]["inputEmail"].value;
          var recepient=document.forms["cartEmailForm"]["inputEmail1"].value;
          var subject=document.forms["cartEmailForm"]["inputtextarea"].value;
          var flag = false;
          
          if((sender==null || sender=="") || (recepient==null || recepient=="") || (subject==null || subject=="")){

	          if(sender==null || sender=="") {
	    	       document.getElementById('errorMsgSender').innerHTML = "Enter sender's address.";
	    	       flag = true
	            }
	          if(recepient==null || recepient==""){
	    	      document.getElementById('errorMsgRecepient').innerHTML = "Enter recepient's address.";
	    	      flag = true
	          }
	          if(subject==null || subject==""){
	    	      document.getElementById('errorMsgSubject').innerHTML = "Subject is empty.";
	    	      flag = true
	          }
          }
          ...
          if (flag){
              ...
              if(this.shoppingCartPage)
            	  params.emailFromPage = "shoppingCartPage";
              else if(this.quoteCartPage)
            	  params.emailFromPage = "quoteCartPage";
            	  
              wc.service.invoke("AjaxSendCartMail",params);
			  // Change to close cart email model.
        	  if($('#myModal') != null && $('#myModal') != 'undefined'){
				$('#myModal').modal('hide');
			  }
          }
      },
	...
	updateShoppingCart:function(form, validateQuantityOnly) {
		...
		if((!$('#qty_2').length > 0) && $('#qty_1').length > 0 && $('#qty_1').val() == 0){
			return;
		}
	...
	updateShipModeForAllItems:function(shipmentSelectBox){
		...
		var callPTMOnlyForSingleShippment = true;
		if(shipModeId == -1){
			return;
		}
		params = [];
		...
		this.updateParamObject(params,"callPTMOnlyForSingleShippment",callPTMOnlyForSingleShippment,false);
	...
	moveAllItemsToSingleShipment:function(){
		...
		if(document.getElementById("MS_requestShippingDateCheckbox_"+orderItemId).checked){
	...
	hideDeleteItemFromCartDialog:function(){
        document.getElementById("dialogbox").style.display = 'none';
        $("#bodyId").removeClass('bs-example-modal-sm');
        $("#bodyId").removeClass('modal-open');
  },
  
  showDeleteItemFromCartDialog:function(orderItemId,forWishlist){
	  if(orderItemId!=""){
		  this.removeOrderItemId = orderItemId;
	  }
	  if(forWishlist!=""){
		  this.removeWishlist = forWishlist;
	  }
	  $("#dialogbox").modal("show");
  },
  deleteItemFromCart:function(){
	  if(this.shoppingCartPage)
		  this.deleteFromCart(this.removeOrderItemId,this.removeWishlist);
	  else if(this.quoteCartPage)
		  this.deleteFromQuoteCart(this.removeOrderItemId);
  },
  //Havi--Show confirmation Dialog on updating item quantity to 0 
  showUpdateItemQuantityFromDialog:function(quantityBox,orderItemId,event,group,count){	  	  	  	  	    
	 setTimeout(dojo.hitch(this,"showUpdateItemQuantityDialogAfterTimeOut",quantityBox,orderItemId,event,group,count),1000);		  
  },
  // TimeOut needed because we are not getting updated quantity 
  showUpdateItemQuantityDialogAfterTimeOut:function(quantityBox,orderItemId,event,group,count){	
		var quantity = (quantityBox.value);
		this.tempUpdateQuanityValues = [];
		if(quantity == "0"){
			  this.tempUpdateQuanityValues[0] = quantityBox;
			  this.tempUpdateQuanityValues[1] = orderItemId;
			  this.tempUpdateQuanityValues[2] = event;
			  this.tempUpdateQuanityValues[3] = group;
			  this.tempUpdateQuanityValues[4] = count;
			  this.tempUpdateQuanityValues[5] = "Not Updated";
			  this.tempUpdateQuanityValues[6] = quantityBox.value;
			  
			  document.getElementById("dialogboxUpdateQantity").className  = 'modal fade bs-example-modal-sm in';
			  document.getElementById("dialogboxUpdateQantity").style.display = 'block';	 
		}
		else{		
			  this.updateCartWaitQuantityBox(quantityBox, orderItemId,event,group,count);
		}
  },
  hideUpdateQuantityDialog:function(){
	  if(this.tempUpdateQuanityValues[5] != "updated"){
		  $("#qty_"+this.tempUpdateQuanityValues[4]).val(this.tempUpdateQuanityValues[6]);
		  if(this.shoppingCartPage){
		    document.location.href = "AjaxCheckoutDisplayView?storeId=" + this.storeId + "&catalogId=" + this.catalogId + "&langId=" + this.langId;
		  }
		  else if(this.quoteCartPage){
			document.location.href = "AjaxQuoteItemDisplayView?storeId=" + this.storeId + "&catalogId=" + this.catalogId + "&langId=" + this.langId;
		  }
	  }
	        
	  //document.getElementById("dialogboxUpdateQantity").style.display = 'none';
     // $("#bodyId").removeClass('bs-example-modal-sm');
     // $("#bodyId").removeClass('modal-open');
  },
  updateCartQuantityFromDialogBox:function(){	  
	  this.tempUpdateQuanityValues[5] = "updated";
	  this.updateCartWaitQuantityBox(this.tempUpdateQuanityValues[0], this.tempUpdateQuanityValues[1],this.tempUpdateQuanityValues[2],this.tempUpdateQuanityValues[3],this.tempUpdateQuanityValues[4]);
  },
  updateCartWaitQuantityBox:function(quantityBox, orderItemId,event,group,count) {
		if(event.keyCode == dojo.keys.TAB)return;
		if(!this.isAjaxCheckOut()){ 
			return;
		}
		...
		this.keyPressCount[orderItemId] = parseInt(this.keyPressCount[orderItemId]) + 1;       
		setTimeout(dojo.hitch(this,"updateCartQuantityBox",quantityBox,orderItemId,this.keyPressCount[orderItemId],group,count),this.updateWaitTimeOut);
	},
  /**
	 * This function updates the total on the shopping cart page when the quantity of an item has been changed. It is used when the "AjaxCheckOut" feature is enabled.
	 * It updates the shopping cart by calling the <code>AjaxUpdateOrderItem</code> service.
	 ...
	 * @param {Integer} orderItemId The ID of the order item to update.
	 * @param {Integer} keyPressCountValue The count of keyPress events. If there are more keyPress events after this event was fired, then this function just returns without doing anything.
	 * @param {Integer} group Qauntity box jump value.
	 * @param {Integer} id to append.	 
	 */	
	updateCartQuantityBox:function(quantityBox, orderItemId,keyPressCountValue,group,count){
		if(keyPressCountValue < this.keyPressCount[orderItemId]){
			//User has pressed one more key..that key press call will update the server..no work for me..
			return;
		}
		...
		if(!isNonNegativeInteger(quantity)){
			TealeafWCJS.createExplicitChangeEvent(quantityBox.id);
			//MessageHelper.formErrorHandleClient(quantityBox,MessageHelper.messages["QUANTITY_INPUT_ERROR"]);
			$("#elmChkBoxError_"+count).html(MessageHelper.messages["CART_QTY_ERROR"]);
		}
			...
			if(!this.validateCartQuantity(quantityBox,group,count))
			{
			$("#ERR2_"+count).show();
			$('#spclMsg_'+count).hide();
			quantity=this.findNearestNumber('qty_'+count,group);
			...
			if(quantity == 0)
			{
				...
				if(this.shoppingCartPage){
					...
					params.orderId = this.quoteId;
					wc.service.invoke("AjaxUpdateQuoteCartOrderItem",params);
				}				
			}
			else
			{	//Quote
	...
	/**
	 * This function to Validate the quantity in quantity box
	 */
validateCartQuantity:function(quantityBox,group,count)
	{
		if(isNaN(quantityBox.value)|| quantityBox.value==null || quantityBox.value.trim()=="")
			{
			return false;
			}
		else if(isNaN(quantityBox.value)){
			return false;
		}
		else if(quantityBox.value%group!=0){
			return false;
		}
		else
		{
	...
	/**
	 * This function Find the nearest number of multiple quantity
	 */
findNearestNumber:function(element,multiple)
	{
	              //variable maxLimit is defining the maximum limit of the Box Quantity.
            var maxLimit = 1000000;
            var curr=parseInt(document.getElementById(element).value);              
            var totalAmount = parseInt(curr+multiple);
            if(curr%multiple == 0){
                  return curr;      
            }
            if(curr>multiple) {                                         
                        var mod=curr%multiple;                    
                        var newVal=curr;                          
                        var i=0;                            
                        for(;i<multiple;i++)    
                        {
                              newVal=newVal+1;                    
                              if(newVal%multiple==0)                    
                              {
                                    break;
                              }
                        }
                        if((newVal-curr)<(curr-(curr-mod))) {    
                                    if(newVal < maxLimit){
                                          return newVal;
                                    } else {
                                          return (curr-mod);
...
/**
	 * This function Created to increment the value of quantity box
	 */
quantityPlus:function(element,count,multiple,call,field){
	// Stop acting like a button
        element.preventDefault();
        // Get its current value
        var currentVal = parseInt($('#'+field+'_'+count).val());
       /* Variable "incrementedValue" is using for box quantity max value validation. */
        var incrementedValue = currentVal + multiple;
        if(incrementedValue < 1000000){
	        	// If is not undefined
	        if (!isNaN(currentVal))
	         {
	            // Increment
	            $('#'+field+'_'+count).val(currentVal + multiple);
	            $('#'+field+'_'+count).trigger(call);
	            
	        } 
	        else 
	        {
	            // Otherwise put a 0 there
	            $('#'+field+'_'+count).val(currentVal);
	            //$('#'+field+'_'+count).trigger(call);
	...
	/**
	 * This function Created to decrement the value of quantity box
	 */
quantityMinus:function(element,count,multiple,call,field){
	   // Stop acting like a button
        element.preventDefault();
        // Get its current value
        var currentVal = parseInt($('#'+field+'_'+count).val());
	...
	,
	cancelCartMail:function(){
    	      document.getElementById('errorMsgSender').innerHTML = "";
    	      document.getElementById('errorMsgRecepient').innerHTML = "";
    	      document.getElementById('errorMsgSubject').innerHTML = "";
    },
	printCommand:function(cartOrReorderID){
                      if ($(".glyphicon")[0] && $(".glyphicon-plus")[0]){
						$.each( $('.'+cartOrReorderID), function(i, cartOrReorderID) {
						   $('.read_More_four', cartOrReorderID).each(function() {
								var productDetailsHeight = parseInt($('.product-details').outerHeight(true)) ? parseInt($('.product-details').outerHeight(true)) : 221;                                                                                          
								$( this ).parent().css( "height", (productDetailsHeight+parseInt($(this).outerHeight(true))+65)+"px" );
                            });
                         });
						$(".product").find( "a" ).hide();
						$('.hover_underline').show();
						if ($('#displayimg_distributor_print').length > 0) {
							document.getElementById('displayimg_distributor_print').style.display = 'block';
							document.getElementById('displayimg_tootltip_print').style.display = 'block';
						}
						window.print();
						$('.product-details').css("height","221px");
						$(".product").find( "a" ).show();
						if ($('#displayimg_distributor_print').length > 0) {
							document.getElementById('displayimg_distributor_print').style.display = 'none';
							document.getElementById('displayimg_tootltip_print').style.display = 'none';
						}
		} else {
						window.print();
		}
    },
    requestQuote:function(orderId){   	
    	var params = [];
		...
		wc.service.invoke("AjaxQuoteRequestForm", params);   
    },
    /**
	 * This function deletes an order item from the Quote cart.
	 * @param {Integer} orderItemId The ID of the order item to delete.
	 */
    deleteFromQuoteCart:function(orderItemId){ 
		if(!this.isAjaxCheckOut())return;
		...
		//For handling multiple clicks---HAvi
		if(!submitRequest()){
			return;
		}   
		cursor_wait();			
		wc.service.invoke("AjaxDeleteQuoteCartOrderItem", params);		
	},
	/**
	* This is called from Checkout Logon to Merge Quote Items with normal shopping cart
	*
	*/
	guestShopperLogonForQuote: function(logonURL, orderMoveURL) {
		...
		var quoteCartURL = "AjaxQuoteItemDisplayView?storeId=" + this.storeId + "&catalogId=" + this.catalogId + "&langId=" + this.langId;
		var completeOrderMoveURL = orderMoveURL+"&URL="+quoteCartURL;
	...
	refreshQuoteCart:function(){
		 if($('.quote-title').css('visibility') === 'visible'){
			  document.location.href = "AjaxQuoteItemDisplayView?storeId=" + this.storeId + "&catalogId=" + this.catalogId + "&langId=" + this.langId;
		 }
	}
}
