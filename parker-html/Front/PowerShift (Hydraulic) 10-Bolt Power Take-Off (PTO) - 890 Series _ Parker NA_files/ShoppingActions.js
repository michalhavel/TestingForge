...
shoppingActionsJS={
	...
	**/
	...
	addToCompare:function(catEntryIdentifier){
		...
		if (currentNumberOfItemsInCompare < parseInt(this.maxNumberProductsAllowedToCompare)) {
			...
			var compareTxt	=	$("#compareMsg").text();	
			var compareMsg	=	compareTxt.substring(0, compareTxt.indexOf("4")).replace(/\d+/,currentNumberOfItemsInCompare)+compareTxt.substring(compareTxt.indexOf("4"),compareTxt.length);
			$("#compareMsg").html(compareMsg);
			...
			if(currentNumberOfItemsInCompare == this.minNumberProductsAllowedToCompare){
				...
				document.getElementById("compareZoneDisplayArea").style.display = "block";
			}
		} else {
	...
	var cookieKey = this.cookieKeyPrefix + this.storeId;
		...
		if(cookieValue != null){
			if(dojo.trim(cookieValue) == ""){
				...
				var compareTxt	=	$("#compareMsg").text();	
				var compareMsg	=	compareTxt.substring(0, compareTxt.lastIndexOf("4")).replace(/\d+/,currentNumberOfItemsInCompare)+compareTxt.substring(compareTxt.lastIndexOf("4"),compareTxt.length);
				$("#compareMsg").html(compareMsg);
			}
			...
			if(currentNumberOfItemsInCompare > 1){
			    ...
			    document.getElementById("compareZoneDisplayArea").style.display = "none";
			}
	...
	clearAll:function(){
			...
			$('.category_section_div :checkbox:enabled').prop('checked', false);
			$("#compareZoneDisplayArea").css("display","none");
			$("#compareMsg").html("Comparing 0 of 4 ");
			var cookieKey = this.cookieKeyPrefix + this.storeId;
	...
	compareProducts:function(categoryIds){
		var url = "CompareProductsDisplay?storeId=" + this.storeId + "&catalogId=" + this.catalogId + "&langId=" + this.langId + "&compareReturnName=" + this.compareReturnName + "&searchTerm="+ this.searchTerm;
		...
		/* Commnted to fix category facet bug.
	...
	* @param {Boolean} changeAttibute If the flow is coming from change attribute functionality
	*
	**/
	replaceItemAjaxHelper : function(catalogEntryId,qty,removeOrderItemId,addressId,shipModeId,physicalStoreId,changeAttribute){
		...
		wc.service.declare({
			...
			,successHandler: function(serviceResponse) {
				...
				if(changeAttribute == true){
					...
					wc.service.invoke("AjaxAddOrderItemUpdate", params2);
		...
		//Delcare service for adding item while changing attributes..
		wc.service.declare({
			id: "AjaxAddOrderItemUpdate",
			actionId: "AjaxAddOrderItemUpdate",
			url: "AjaxOrderChangeServiceItemAdd",
			formId: ""

			,successHandler: function(serviceResponse) {
				document.location.href="OrderCalculate?calculationUsageId=-1&updatePrices=1&catalogId="+WCParamJS.catalogId+"&errorViewName=AjaxOrderItemDisplayView&orderId=.&langId="+WCParamJS.langId+"&storeId="+WCParamJS.storeId+"&URL=AjaxCheckoutDisplayView";
			}
			,failureHandler: function(serviceResponse) {
				MessageHelper.displayErrorMessage(serviceResponse.errorMessageKey);
			}
	...
	/**Havi
	* replaceQuoteItemAjaxHelper This function is used to replace an item in the Quote cart. This will be called from the {@link fastFinderJS.ReplaceItemAjax} method.
	*
	* @param {String} catalogEntryId The catalog entry of the item to replace to the cart.
	* @param {int} qty The quantity of the item to add.
	* @param {String} removeOrderItemId The order item ID of the catalog entry to remove from the cart.
	...
	*
	**/
	replaceQuoteItemAjaxHelper : function(catalogEntryId,qty,removeOrderItemId,physicalStoreId,changeAttribute){
		...
		//Delcare service for deleting item...
		wc.service.declare({
			id: "AjaxReplaceQuoteItem",
			actionId: "AjaxReplaceQuoteItem",
			url: "AjaxDeleteQuoteCartOrderItem",
			formId: ""

			,successHandler: function(serviceResponse) {
				//Now add the new item to cart..
				wc.service.invoke("AjaxAddQuoteItemUpdate", params2);				
			}
		...
		//Delcare service for adding item..
		wc.service.declare({
			id: "AjaxAddQuoteItemTemp",
			actionId: "AjaxAddQuoteItemTemp",
			url: "AjaxQuoteOrderChangeServiceItemAdd",
			formId: ""
		...
		//Delcare service for adding item while changing attributes..
		wc.service.declare({
			id: "AjaxAddQuoteItemUpdate",
			actionId: "AjaxAddQuoteItemUpdate",
			url: "AjaxQuoteOrderChangeServiceItemAdd",
			formId: ""

			,successHandler: function(serviceResponse) {
				document.location.href="AjaxQuoteItemDisplayView?catalogId="+WCParamJS.catalogId+"&langId="+WCParamJS.langId+"&storeId="+WCParamJS.storeId;
			}
			,failureHandler: function(serviceResponse) {
				MessageHelper.displayErrorMessage(serviceResponse.errorMessageKey);
			}
		...
		cursor_wait();
		wc.service.invoke("AjaxReplaceQuoteItem",params);
	},
	/**
